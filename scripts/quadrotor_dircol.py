...
def funny_traj(plant, init_state, manual_points,minimum_timesteo=0.1,maximum_timestep=1):
    x_traj = None
    u_traj = None
    ...
    dircol = DirectCollocation(
        ...
        minimum_timestep=minimum_timesteo,
...
def traj_opt(plant, init_state, final_state, num_time_samples=21, minimum_time=0.5, maximum_time=50.0, StateView=None):
    ...
    dircol = DirectCollocation( #FIX ARG NAMES TO COMPILE?
        ...
        minimum_timestep=minimum_time / num_time_samples,
        maximum_timestep=maximum_time / num_time_samples,
    )
    #input constraints
    prop_limit = 2.0 #no idea unit T_T
    u = dircol.input() #input decision variables
    for i in range(4):
        dircol.AddConstraintToAllKnotPoints(u[i] <= prop_limit)
        dircol.AddConstraintToAllKnotPoints(-prop_limit <= u[i])
    ...
    initial_x_trajectory = PiecewisePolynomial.FirstOrderHold(
        [0.0, 4.0], np.column_stack((init_state, final_state))
    )
...
def MakeMultibodyQuadrotorLQR():
    ...
    x_traj, u_traj, ax = traj_opt(diagram, init_state, final_state, StateView=StateView)
    options.x0 = x_traj
    options.u0 = u_traj
    ...
    for i in range(5):
        ...
        initial_state.x_x += rng.standard_normal() * 2
        initial_state.y_x += rng.standard_normal() * 2
